...
export async function GET(request: NextRequest) {
  ...
  try {
    const res = await fetch(`${adminApiUrl()}/api/admin/signals-recent?limit=${encodeURIComponent(limit)}`, {
    ...
    return NextResponse.json({ error: 'Failed to fetch recent signals', details: String(error) }, { status: 500 })
  }
}
