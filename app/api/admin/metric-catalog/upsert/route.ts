...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const res = await fetch(`${adminApiUrl}/api/admin/metric-catalog/upsert`, {
      method: 'POST',
      headers: {
    ...
    return NextResponse.json({ error: 'Failed to proxy metric catalog upsert', details: String(error) }, { status: 500 })
  }
}
