...
export async function POST(
  ...
  try {
    ...
    const res = await fetch(`${adminApiUrl}/api/admin/wiki/notes/${encodeURIComponent(id)}/review`, {
      method: 'POST',
      headers: {
    ...
    console.error('[admin/api/wiki/notes/:id/review] Error:', error)
    return NextResponse.json({ error: 'Failed to update backend', details: String(error) }, { status: 500 })
  }
}
