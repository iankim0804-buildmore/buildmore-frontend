...
export async function POST() {
  ...
  try {
    const res = await fetch(`${adminApiUrl()}/api/admin/wiki-graph/lint`, {
      method: 'POST',
      headers: { 'X-Internal-API-Key': auth.internalKey },
      cache: 'no-store',
    })
    if (!res.ok) {
    ...
    console.error('[admin/api/wiki-graph/lint] Error:', error)
