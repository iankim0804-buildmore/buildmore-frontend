...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const url = `${adminApiUrl}/api/admin/source-collectors/run?mode=${encodeURIComponent(mode)}`
    ...
    return NextResponse.json({ error: 'Failed to run source collectors', details: String(error) }, { status: 500 })
  }
}
