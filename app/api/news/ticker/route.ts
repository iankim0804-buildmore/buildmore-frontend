...
export async function GET() {
  ...
  try {
    ...
    if (!res.ok) return NextResponse.json({ items: [], total: 0, generated_at: new Date().toISOString() }, { status: res.status })
    return NextResponse.json(await res.json())
  } catch {
    return NextResponse.json({ items: [], total: 0, generated_at: new Date().toISOString() }, { status: 502 })
  }
}
