...
const heroStats: Array<{ value: string; label: string; sub: string }> = [
  { value: "1 → 3", label: "주소 하나, 시나리오 셋", sub: "리모델링 · 증축 · 신축 동시 비교" },
  { value: "08", label: "밸류애드 전략", sub: "정비사업부터 계약까지" },
  { value: "360°", label: "리스크 점검", sub: "인허가 · 세금 · 법률 · 계약" },
  { value: "Daily", label: "데이터 갱신", sub: "공공 · 민간 원천데이터 수집" },
]
...
function SectionLabel({ no, en }: { no: string; en: string }) {
  return (
    <p className="flex items-center gap-3 text-[11px] font-semibold uppercase tracking-[0.32em] text-emerald-400">
      <span className="font-mono text-zinc-500">{no}</span>
    ...
    <main className="min-h-screen break-keep bg-[#0a0d12] text-zinc-100 antialiased">
      ...
      >
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 sm:h-[72px] sm:px-8">
          <Link href="/landing" className="flex items-baseline gap-1 text-[15px] font-bold tracking-[0.18em]" aria-label="BuildMore 홈">
            BUILDMORE
      ...
      <section className="relative flex min-h-svh flex-col justify-end overflow-hidden">
            ...
            <div className="grid grid-cols-2 border-t border-white/15 lg:grid-cols-4">
              {heroStats.map((stat, i) => (
                ...
                >
                  ...
                  <p className="mt-1 text-xs leading-5 text-zinc-500">{stat.sub}</p>
                </div>
              ))}
            </div>
          </Reveal>
        ...
        <a
          href="#manifesto"
          className="absolute bottom-6 right-6 hidden items-center gap-2 text-[11px] font-medium uppercase tracking-[0.3em] text-zinc-500 transition-colors hover:text-zinc-300 md:flex"
        >
          Scroll
        ...
        <div className="mx-auto max-w-7xl px-5 py-24 sm:px-8 sm:py-36">
          ...
          <Reveal delay={100}>
            <h2 className="mt-10 max-w-4xl text-4xl font-bold leading-[1.15] tracking-tight text-white sm:text-5xl lg:text-6xl">
              기다리는 투자가 아니라,
              <br />
              <span className="text-zinc-500">설계하는 투자.</span>
            </h2>
          </Reveal>
          <Reveal delay={200}>
            ...
            {solutionDomains.map((domain, index) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="mt-16 text-[10px] font-semibold uppercase tracking-[0.28em] text-zinc-500">
                    {domain.en}
                  </p>
                  ...
                  <ul className="mt-6 space-y-2.5 border-t border-white/10 pt-5">
                    {domain.items.map((item) => (
                      <li key={item} className="flex items-start gap-2.5 text-[13px] leading-5 text-zinc-500">
            ...
            {strategies.map((strategy, index) => (
              ...
              >
                ...
                <div>
                  ...
                  <p className="mt-2.5 max-w-md text-sm leading-7 text-zinc-500">{strategy.desc}</p>
                </div>
              </Reveal>
            ))}
          </div>
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    {engine.href ? (
                      <Link
                        href={engine.href}
                        className="flex items-center gap-1.5 text-xs font-semibold text-zinc-500 transition-colors hover:text-emerald-300"
                      >
                        {engine.linkLabel}
          ...
          <div className="mt-14 grid gap-px border border-white/10 bg-white/10 md:grid-cols-2 lg:grid-cols-4">
            {processSteps.map((step, index) => (
              <Reveal key={step.no} delay={index * 90} className="relative bg-[#0d1117] p-7 sm:p-8">
                ...
                <p className="mt-3 text-sm leading-7 text-zinc-500">{step.desc}</p>
              ...
              <table className="w-full min-w-[720px] border-collapse text-left">
                <thead>
                  <tr className="bg-white/[0.04]">
                    <th className="border-b border-r border-white/10 p-5 text-xs font-semibold uppercase tracking-[0.2em] text-zinc-500">
                      비교 항목
                    </th>
                ...
                <tbody>
                  {scenarioRows.map(([label, remodel, extend, rebuild]) => (
                    <tr key={label} className="transition-colors hover:bg-white/[0.02]">
                      <th className="border-r border-t border-white/10 p-5 text-sm font-medium text-zinc-500">{label}</th>
                      <td className="border-r border-t border-white/10 bg-emerald-400/[0.04] p-5 font-mono text-[15px] font-semibold text-white">
                        {remodel}
                      </td>
                      <td className="border-r border-t border-white/10 p-5 font-mono text-[15px] text-zinc-400">{extend}</td>
                      <td className="border-t border-white/10 p-5 font-mono text-[15px] text-zinc-400">{rebuild}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
